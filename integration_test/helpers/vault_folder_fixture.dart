...
class VaultFolderFixture {
  ...
  VaultFolderFixture._({required this.rootPath, required Directory tempDir}) : _tempDir = tempDir;
  ...
  Future<String> readFile(String relativePath) => File(p.join(rootPath, relativePath)).readAsString();

  Future<bool> fileExists(String relativePath) => File(p.join(rootPath, relativePath)).exists();

  Future<List<String>> listPages() async {
    final pagesDir = Directory(p.join(rootPath, 'pages'));
    if (!await pagesDir.exists()) return [];
    return (await pagesDir.list(recursive: true).toList()).whereType<File>().where((f) => f.path.endsWith('.md')).map((f) => p.relative(f.path, from: rootPath)).toList();
  }
