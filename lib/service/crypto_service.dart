  ...
  Future<IdentityKeyPair> deriveIdentityKeyPair(List<int> entropy32) async {
    if (entropy32.length != 32) {
      throw ArgumentError.value(
        entropy32.length,
        'entropy32',
        'Identity entropy MUST be 32 bytes',
      );
    }

    final hkdf = Hkdf(hmac: Hmac.sha256(), outputLength: 32);
    final edSeed = await hkdf.deriveKey(
      secretKey: SecretKey(Uint8List.fromList(entropy32)),
      nonce: utf8.encode('noetec.identity.v1'),
      info: utf8.encode('noetec-identity-key'),
    );
    ...
    return IdentityKeyPair(
      publicKeyBase64Url: base64UrlEncodeNoPad(publicKeyBytes),
      privateKeyBase64Url: base64UrlEncodeNoPad(edSeed.bytes),
    );
  }
}
